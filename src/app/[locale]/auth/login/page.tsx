...
export default function LoginPage({
  ...
  const oauthErrors: Record<string, string> = {
    google_failed: isFrench
      ? "La connexion Google a échoué. Veuillez réessayer."
      : "Google login failed. Please try again.",
    user_not_found: isFrench
      ? "Aucun compte trouvé avec cet email Google."
      : "No account found with this Google email.",
    admin_google_forbidden: isFrench
      ? "Les administrateurs doivent utiliser l'authentification par email/mot de passe."
      : "Administrators must use email/password authentication.",
    account_locked: isFrench
      ? "Votre compte est verrouillé. Contactez le support."
      : "Your account is locked. Contact support.",
    callback_failed: isFrench
      ...
      <div className="w-full max-w-md">
        ...
        >
          ...
          <AnimatePresence>
            {errorParam && oauthErrors[errorParam] && (
              <motion.div
                ...
                className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg p-3 mb-6 flex items-start gap-3"
              >
                <AlertCircle className="h-5 w-5 text-red-600 dark:text-red-400 flex-shrink-0 mt-0.5" />
                <p className="text-sm text-red-700 dark:text-red-300">
                  {oauthErrors[errorParam]}
                </p>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            ...
            <MarketplaceButtonLink href={`/${locale}/auth/register`}>
